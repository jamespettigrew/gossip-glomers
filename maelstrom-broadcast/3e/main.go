...
func (s *server) HandleInitMsg(msg maelstrom.Message) error {
	...
	// Follower nodes should only broadcast to the leader
	if s.id != "n0" {
		neighbourIds = append(neighbourIds, "n0")
	} else {
		for _, neighbourId := range s.node.NodeIDs() {
			// n0 is the designated leader for the cluster
			// If this is the leader node then don't broadcast to self
			if neighbourId == "n0" {
				continue
			}
